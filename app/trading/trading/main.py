...
class Trading:
    def __init__(self):
        ...
        self.num_consumers = 3  # The number of consumers can be adjusted as needed
        self.swap_event_consumers = []
        for i in range(self.num_consumers):
            consumer = SwapEventConsumer(
                self.redis,
                "trading:swap_event",
                f"trading:new_swap_event:{i}",  # Create a unique name for each consumer
            )
    ...
    async def _process_single_swap_event(self, swap_event: SwapEvent):
        """Core logic for processing a single transaction event"""
    ...
    async def _process_swap_event(self, swap_event: SwapEvent):
        """Create a new task to process transaction events"""
